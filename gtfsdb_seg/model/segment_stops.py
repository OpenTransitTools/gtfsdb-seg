...
class SegmentStops(Base, PatternBase):
    __tablename__ = 'seg_stops'
    ...
    def __init__(self, session, id, begin_stop, end_stop, trip):
        super(SegmentStops, self).__init__()
        self.id = id
    ...
    def _cache_segment(cls, session, begin_stop, end_stop, trip, segment_cache, segment_trip_cache):
        ...
        if segment_trip_cache is not None:
            ...
            sst = SegmentTrips(session, stop_segment, trip)
    ...
    def to_geojson(cls, session):
        ...
        #import pdb; pdb.set_trace()
        features = session.query(SegmentStops.id, SegmentStops.direction, SegmentStops.geom.ST_AsGeoJSON(),
                                 SegmentStops.begin_stop_id, SegmentStops.end_stop_id).all()
        ln = len(features) - 1
        featgeo = ""
        last_stop = "xxx"
